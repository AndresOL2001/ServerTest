...
const Navbar: React.FC<INavbar> = ((props: INavbar) => {
  const {logo, showLogo, showSearchBar, items, userNav, userInfo, backgroundColor, textColor, primary} = props

  const renderListOfMenu = (items:string[], styles?: string) => {
    return items?.map(item => <li className={styles}><Link href=''></Link>{item}</li>)
  }

  return (
    <>
      <div className='flex justify-center font-sans' style={{backgroundColor, color: textColor}}>
              ...
              primary ? (
                ...
                <>
                  <ul className={showSearchBar ? 'flex lg:gap-6 sm:gap-2 text-light items-center text-base' : 'flex lg:gap-14 sm:gap-6 text-light text-base items-center'}>
                    ...
                    {renderListOfMenu(items, showSearchBar ? 'py-5 font-sans' : 'last-of-type:bg-blue-primary lg:last-of-type:px-8 sm:last-of-type:px-2 lg:py-5 sm:py-3 sm:text-center font-sans')}
                  </ul>
